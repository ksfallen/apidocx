...
public class PsiClassUtils {
    ...
    protected PsiClassUtils(@NotNull PsiClass psiClass) {
        this.psiClass = psiClass;
    }

    public static PsiClassUtils create(@NotNull PsiClass psiClass) {
        return new PsiClassUtils(psiClass);
    }
    ...
    private static boolean isListFieldType(PsiType psiFieldType) {
        ...
        for (PsiType psiType : ((PsiClassReferenceType) psiFieldType).rawType()
                .getSuperTypes()) {
    ...
    private static boolean isEnum(PsiType psiFieldType) {
        ...
        return Objects
                .requireNonNull(((PsiClassReferenceType) psiFieldType).resolve())
                .isEnum();
    }
    ...
    public String convertClassToJSON(String className, String paramName, Project project) {
        ...
        if (className.contains("List<")) {
            ...
            // build RequestBody Json
            int gIdx = className.indexOf("<");
            if (gIdx != -1) {
                // For example: PageParam<VetReq> req
                // innerGenericName: VetReq；outerGenericName：PageParam
                String innerGenericName = className.substring(
                        gIdx + 1,
                        className.lastIndexOf(">")
                );
                String outerGenericName = className.substring(0, gIdx);

                Map<String, Object> innerMap = assembleClassToMap(innerGenericName, project);
                Map<String, Object> outerMap = assembleClassToMap(outerGenericName, project);

                Map<String, Object> flatMap = new LinkedHashMap<>(outerMap);
                flatMap.put(paramName, innerMap);
                queryJson = gson.toJson(flatMap);
            } else {
    ...
    @Nullable
    protected PsiClass findOnePsiClassByClassName_deprecated(
            String className,
            Project project
    ) {
        ...
        Collection<PsiClass> psiClassCollection = tryDetectPsiClassByShortClassName(
                project,
                shortClassName
        );
        ...
        if (psiClassCollection.size() > 1) {
            ...
            Optional<PsiClass> any = psiClassCollection
                    .stream()
    ...
    public Collection<PsiClass> tryDetectPsiClassByShortClassName(
            Project project,
    ...
    public PsiClass[] tryDetectPsiClassByShortClassName2(
            String shortClassName,
            Project project
    ) {
        // all
    ...
    public Map<String, Object> assembleClassToMap(
            PsiClass psiClass,
            Project project,
            int recursiveCount
    ) {
        ...
        for (PsiField field : fields) {
            ...
            if (psiFieldType instanceof PsiArrayType) {
                ...
                Object baseTypeDefaultValue = getJavaBaseTypeDefaultValue(
            ...
            String qualifiedName = resolveClass.getQualifiedName();

            //            self recursion
            if (Objects.equals(psiClass.getQualifiedName(), qualifiedName)) {
                if (recursiveCount > 0) {
            ...
            if (isListFieldType(psiFieldType)) {
                ...
                if (parameters.length > 0) {
                    ...
                    if (parameter instanceof PsiClassReferenceType) {
                        ...
                        PsiClass onePsiClassByClassName = findOnePsiClassByClassName(
                                parameter.getCanonicalText(),
                                project
                        );

                        Map<String, Object> objectMap = assembleClassToMap(
                                onePsiClassByClassName,
                                project,
                                0
                        );
                        map.put(fieldName, Lists.newArrayList(objectMap));
    ...
    public PsiClassUtils withModule(Module module) {
